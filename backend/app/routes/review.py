...
async def get_review_me(
    ...
    stmt_review = select(Review).where(
        and_(Review.user_id == current_user.id, Review.song_id == song.id)
    )
...
async def create_review(
    ...
    existing_review = await db.scalar(
        select(Review).where(
            Review.user_id == current_user.id, Review.song_id == target_song_id
        )
    )

    if existing_review:
        raise HTTPException(
            status_code=400, detail="คุณเคยรีวิวเพลงนี้ไปแล้ว หากต้องการแก้ไขให้ใช้ PUT แทนครับ"
        )
...
async def get_my_reviews(
    db: SessionDep, current_user: User = Depends(get_current_active_user)
):
